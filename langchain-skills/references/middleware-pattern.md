...
Complete implementation pattern for LangChain AgentMiddleware.
...
class SkillsMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        self.user_skills_display = f"~/.{assistant_id}/skills"
...
from your_skills_module import SkillsMiddleware

def create_agent(assistant_id: str, enable_skills: bool = True):
    agent_middleware = []

    if enable_skills:
        skills_dir = Path.home() / f".{assistant_id}" / "skills"
        project_skills_dir = Path.cwd() / f".{assistant_id}" / "skills"

        agent_middleware.append(
            SkillsMiddleware(
                skills_dir=skills_dir,
                assistant_id=assistant_id,
                project_skills_dir=project_skills_dir if project_skills_dir.exists() else None,
            )
        )

    # Create your agent with middleware
    agent = create_your_agent(middleware=agent_middleware)
    return agent
```
